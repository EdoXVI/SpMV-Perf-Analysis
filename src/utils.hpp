...
inline int readMtx(const char *fname, std::vector<I> *row_indices,
		...
		I* num_cols, I* num_nnz, int * directed, bool read_values = false,
    ...
    // Read MTX graph orientation
    bool is_undirected = mm_read_mtx_is_undirected(f);
    //std::cout << is_undirected << std::endl;
